...
tab_scdf <- nav_panel(
  ...
  layout_sidebar(
    sidebar = sidebar(
      ...
      textAreaInput(
        "new_values", "Values (dependent variable)",
...
tab_transform <- layout_sidebar(
  sidebar = sidebar(
    ...
    div(style = "display:inline-block; vertical-align: top",
        textInput(
          "select_phasesA", "Combine phases to A",
          placeholder = "(e.g.: 1)"
        )
    ),
    div(style = "display:inline-block; vertical-align: top; padding-left: 30px;",
        textInput(
          "select_phasesB", "Combine phases to B",
          placeholder = "(e.g.: 2,3)"
        )
    ),
    textInput(
    ...
    textInput(
      "setdvar", "Set dependent variable",
...
tab_plot <- layout_sidebar(
  sidebar = sidebar(
    textAreaInput(
      "plot_arguments", "Arguments",
      value = "", rows = 5,
      placeholder = res$placeholder$plot_arguments
    ),
    selectInput(
      "scplot_examples", "Stats templates",
      choices = names(res$choices$scplot_examples)
    ),
    selectInput(
      "scplot_templates_design", "Design templates",
      choices = names(res$choices$scplot_templates_design)
    ),
    selectInput(
      "scplot_templates_annotate", "Annotate templates",
...
#------------------------
# Power-test (kept as grid; works with BS5)
#------------------------
tab_power_test <- fluidRow(
  column(2, div(style = res$div$pt,
                ...
                selectInput(
                  "design_distribution", "Distribution",
...
#------------------------
# Settings (kept as grid; works with BS5)
#------------------------
tab_settings <- fluidRow(
  column(2, div(
    style = res$div$settings,
    h3("Data"),
    input_switch("scdf_output_format", "Show scdf syntax1", value = FALSE),
    input_switch(
  ...
  column(2, div(
    ...
    textInput(
      "transform_save_prefix", "Prefix save filename",
  ...
  column(2, div(
    ...
    radioButtons(
      "rename_predictors", "Rename predictors",
      choices = c("full", "concise", "no"), inline = TRUE
    ),
    radioButtons(
      "format_output_stats", "Save format",
  ...
  column(2, div(
    ...
    h3("General"),
    radioButtons(
      "scan_export_engine", "Html engine",
...
ui <- page_navbar(
  title = "scan",
  theme = res$theme_light,                      # assumed BS5 bslib theme from resources.R
