...
#' single-case plot
#'
#' @param style A character string or a vector of character strings with
#'   predefined styles.
#' @param ... Further arguments passed to the plot command.
#'
#' @return Returns a list to be provided for the style argument of the
#'   [plot.scdf()] function.
#' @return \itemize{ \item\code{fill} If set, the area under the line is filled
#'   with the given color (e.g., \code{fill = "tomato"}). Use the standard R
#'   command colors() to get a list of all possible colours. \code{fill} is
...
#'   in brown color to the plot. \item\code{"names"} A list of parameters
#'   defining the depiction of phase names (e.g. \code{names = list(cex = 0.8,
#'   col = "red", side = 1)}: cex for size, col for color, and side for
#'   position). See \code{\link{mtext}} for more details. \item\code{"lwd"}
#'   Width of the plot line. Default is \code{lwd = 2}. \item\code{"pch"} Point
#'   type. Default is \code{pch = 17} (triangles). Other options are for
#'   example: 16 (filled circles) or "A" (uses the letter A). \item\code{"main"}
#'   Main title of the plot. \item\code{"mai"} Sets the margins of the plot.
#'   \item\code{"bty"} Shape of the frame surrounding the inner plot
#'   \item\code{"fill.bg"} Background color of the plot. If a vector is
#'   provided, these colors will be assigned to phases (each phase name becomes
#'   a color). \item\code{"grid"} Color of a grid. \item\code{"text.ABlag"} Text
#'   displayed between phases. \item\code{"cex.axis"} Size of the axis
#'   annotations \item\code{"las"} Orientation of the axis annotations
#'   \item\code{"col.lines"} Color of the lines \item\code{"col.dots"} Color of
#'   the dots \item\code{"col.seperator"} Color of the phase seperating lines
#'   \item\code{"col.bg"} Color of the outer plot \item\code{"col"} General
#'   color setting for the plot \item\code{"col.text"} Color of all labels of
#'   the plot. }
