...
.check_in <- function(x, arg, match) {
  ...
  msg <- paste0("'", match, "'")
  if (length(match) == 1) msg <- msg
  if (length(match) == 2) msg <- paste0(msg, collapse = " or ")
  if (length(match) > 2) 
    msg <- paste0("one of ", paste0(msg, collapse = ", "))
  .check(x, arg %in% match, as.character(match.call()[3]), " not ", msg, ".")
}

.check_within <- function(x, arg, lower, upper) {
  x <- .check_not(x, any(arg<lower), as.character(match.call()[3]), " < ", lower) 
  x <- .check_not(x, any(arg>upper), as.character(match.call()[3]), " > ", upper) 
  x
}
